...
#    d_l1_p = tf.nn.max_pool(d_l1_a, max_s, max_s, padding='VALID') # 128
    ...
    d_l2_a = tf.layers.conv2d(frames, 64, 3, activation=tf.nn.relu, padding='same')
    d_l2_p = tf.nn.max_pool(d_l2_a, max_s, max_s, padding='VALID') # 64

    d_l3_a = tf.layers.conv2d(d_l2_p, 64, 3, activation=tf.nn.relu, padding='same')
    d_l3_p = tf.nn.max_pool(d_l3_a, max_s, max_s, padding='VALID') # 32

    d_l4_a = tf.layers.conv2d(d_l3_p, 128, 3, activation=tf.nn.relu, padding='same')
    d_l4_p = tf.nn.max_pool(d_l4_a, max_s, max_s, padding='VALID') # 16

    d_l5_a = tf.layers.conv2d(d_l4_p, 128, 3, activation=tf.nn.relu, padding='same')
    d_l5_p = tf.nn.max_pool(d_l5_a, max_s, max_s, padding='VALID') # 8

    d_l6_a = tf.layers.conv2d(d_l5_p, 256, 3, activation=tf.nn.relu, padding='same')
    d_l6_p = tf.nn.max_pool(d_l6_a, max_s, max_s, padding='VALID') # 4

    d_l7_a = tf.layers.conv2d(d_l6_p, 256, 3, activation=tf.nn.relu, padding='same')
    d_l7_p = tf.nn.max_pool(d_l7_a, max_s, max_s, padding='VALID') # 2

    d_l8_a = tf.layers.conv2d(d_l7_p, 512, 3, activation=tf.nn.relu, padding='same')
    d_l8_p = tf.nn.max_pool(d_l8_a, max_s, max_s, padding='VALID') # 1

    final_state = tf.concat([d_l8_p, ca, tl], -1)
    latent = tf.layers.dense(final_state, 512, activation=tf.nn.relu)
    ...
    u_l7_a = tf.layers.conv2d_transpose(u_l8_s, 1024, 3, 2, activation=tf.nn.relu, padding='same') # 4
    u_l7_c = tf.concat([u_l7_a, d_l7_a], -1)
    u_l7_s = tf.layers.conv2d(u_l7_c, 1024, 3, activation=tf.nn.relu, padding='same')

    u_l6_a = tf.layers.conv2d_transpose(u_l7_s, 512, 3, 2, activation=tf.nn.relu, padding='same') # 8
    u_l6_c = tf.concat([u_l6_a, d_l6_a], -1)
    u_l6_s = tf.layers.conv2d(u_l6_c, 512, 3, activation=tf.nn.relu, padding='same')

    u_l5_a = tf.layers.conv2d_transpose(u_l6_s, 256, 3, 2, activation=tf.nn.relu, padding='same') # 16
    u_l5_c = tf.concat([u_l5_a, d_l5_a], -1)
    u_l5_s = tf.layers.conv2d(u_l5_c, 256, 3, activation=tf.nn.relu, padding='same')

    u_l4_a = tf.layers.conv2d_transpose(u_l5_s, 128, 3, 2, activation=tf.nn.relu, padding='same') # 32
    u_l4_c = tf.concat([u_l4_a, d_l4_a], -1)
    u_l4_s = tf.layers.conv2d(u_l4_c, 128, 3, activation=tf.nn.relu, padding='same')

    u_l3_a = tf.layers.conv2d_transpose(u_l4_s, 64, 3, 2, activation=tf.nn.relu, padding='same') # 64
    u_l3_c = tf.concat([u_l3_a, d_l3_a], -1)
    u_l3_s = tf.layers.conv2d(u_l3_c, 64, 3, activation=tf.nn.relu, padding='same')

    u_l2_a = tf.layers.conv2d_transpose(u_l3_s, 64, 3, 2, activation=tf.nn.relu, padding='same') # 128
    u_l2_c = tf.concat([u_l2_a, d_l2_a], -1)
    u_l2_s = tf.layers.conv2d(u_l2_c, 64, 3, activation=tf.nn.relu, padding='same')
